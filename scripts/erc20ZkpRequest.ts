...
const main = async () => {
  ...
  const ERC20VerifierAddress = "0xB205054785dd17F9c7F52e1cc3D52FF75F95aEf3";
  ...
  const query = {
    ...
    // 5 = not-in
    operator: 2,
    // 18 refers to the number in years we're using for our proof request
    // - see proofRequest.ts L489
    value: [18, ...new Array(63).fill(0).map((i) => 0)], // the value must be 1 = true
    circuitId,
  };
